...
# Ссылка на отзывы. Можно переопределить через .env: REVIEWS_URL=https://t.me/your_reviews_channel
REVIEWS_URL = _read_env_value("REVIEWS_URL") or "https://t.me/FelecasterNews"
...
def admin_menu():
    ...
    buttons = [
        ...
        ("📱 QR-код оплаты", "admin_qr_payment"),
...
class QRPaymentState(StatesGroup):
    photo = State()
...
async def menu_reviews(callback: CallbackQuery):
    builder = InlineKeyboardBuilder()
    builder.button(text="⭐ Смотреть отзывы", url=REVIEWS_URL)
    builder.button(text="🏠 Главное меню", callback_data="menu_home")
    builder.adjust(1)

    await callback.message.answer(
        "⭐ <b>ОТЗЫВЫ FELECASTER SHOP</b>\n\n"
        "Здесь можно посмотреть отзывы покупателей.",
        reply_markup=builder.as_markup(),
        parse_mode="HTML",
    )
    await callback.answer()
...
def payment_methods_keyboard(product_id=None):
    ...
    builder.button(text="💵  USDT / Crypto Bot", callback_data=f"paymethod:usdt{suffix}")
    builder.button(text="📱  Оплата по QR-коду", callback_data=f"paymethod:qr{suffix}")
...
def qr_payment_text(order_id, total):
    return (
        "📱 <b>ОПЛАТА ПО QR-КОДУ</b>\n\n"
        f"Заказ: <b>#{order_id}</b>\n"
        f"Сумма: <b>{format_price(total, 'RUB')}</b>\n\n"
        "1️⃣ Откройте приложение банка и отсканируйте QR-код.\n"
        "2️⃣ Проверьте сумму и реквизиты.\n"
        "3️⃣ Оплатите заказ.\n"
        "4️⃣ После оплаты отправьте сюда скриншот платежа.\n\n"
        "⚠️ <b>Без скриншота заявка на проверку не отправится.</b>"
    )


def get_saved_qr_photo_id():
    """Возвращает Telegram file_id QR-кода, загруженного администратором."""
    try:
        cursor.execute("SELECT value FROM bot_settings WHERE key='payment_qr_photo_id'")
        row = cursor.fetchone()
        return row[0] if row and row[0] else None
    except Exception:
        return None
...
def qr_payment_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="🔗 Открыть оплату", url=SBP_PAYMENT_URL)
    return builder.as_markup()


def create_payment_qr():
    """Резервный QR из ссылки, если администратор ещё не загрузил свой QR."""
    qr = qrcode.QRCode(version=None, box_size=10, border=4)
    qr.add_data(SBP_PAYMENT_URL)
    qr.make(fit=True)
    image = qr.make_image()
    qr_path = Path(__file__).resolve().with_name("generated_sbp_qr.png")
    image.save(qr_path)
    return qr_path
...
async def choose_payment_method(callback: CallbackQuery, state: FSMContext):
    ...
    payment_text = qr_payment_text(order_id, total) if method == "qr" else manual_payment_text(order_id, currency, total)
    if promo_discount and promo_code:
    ...
    # RUB и QR остаются на ручной схеме со скриншотом.
    if method == "qr":
        try:
            saved_qr = get_saved_qr_photo_id()
            if saved_qr:
                await callback.message.answer_photo(
                    photo=saved_qr,
                    caption=payment_text,
                    reply_markup=qr_payment_keyboard(),
                    parse_mode="HTML",
                )
            else:
                ...
                await callback.message.answer_photo(
                    ...
                    caption=payment_text + "\n\nℹ️ <i>Администратор ещё не загрузил свой QR — используется резервный QR.</i>",
                    reply_markup=qr_payment_keyboard(),
                    parse_mode="HTML",
                )
        except Exception as e:
            print(f"[QR CREATE ERROR] order={order_id}: {e}")
            await callback.message.answer(
                payment_text,
                reply_markup=qr_payment_keyboard(),
                parse_mode="HTML",
    ...
    await callback.bot.send_message(ADMIN_ID,
        ...
        f"Номер: <b>#{order_id}</b>\n"
        f"Покупатель: <code>{callback.from_user.id}</code>\n"
        f"Способ: <b>{'QR-код' if method == 'qr' else currency}</b>\n"
...
@dp.callback_query(F.data == "admin_qr_payment")
async def admin_qr_payment(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        await callback.answer()
        return
    current = get_saved_qr_photo_id()
    builder = InlineKeyboardBuilder()
    if current:
        builder.button(text="🗑 Удалить QR-код", callback_data="admin_qr_delete")
    builder.button(text="⬅️ Админ-панель", callback_data="admin_back")
    builder.adjust(1)
    await state.set_state(QRPaymentState.photo)
    await callback.message.answer(
        "📱 <b>QR-КОД ОПЛАТЫ</b>\n\n"
        f"Текущий QR: <b>{'✅ установлен' if current else '❌ не установлен'}</b>\n\n"
        "Просто отправьте сюда <b>фото QR-кода</b> обычным сообщением.\n"
        "Бот сохранит его и будет показывать покупателям при выборе «📱 Оплата по QR-коду».\n\n"
        "💡 Фото можно отправить без подписи.",
        parse_mode="HTML", reply_markup=builder.as_markup())
    await callback.answer()


@dp.message(QRPaymentState.photo)
async def save_payment_qr(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    if not message.photo:
        await message.answer("❌ Отправьте именно <b>фото QR-кода</b>.", parse_mode="HTML")
        return
    photo_id = message.photo[-1].file_id
    cursor.execute("""
        INSERT INTO bot_settings(key, value) VALUES('payment_qr_photo_id', ?)
        ON CONFLICT(key) DO UPDATE SET value=excluded.value
    """, (photo_id,))
    db.commit()
    await state.clear()
    await message.answer(
        "✅ <b>QR-код оплаты сохранён!</b>\n\n"
        "Теперь этот QR будет показываться покупателям при выборе оплаты по QR-коду.",
        parse_mode="HTML", reply_markup=admin_menu())


@dp.callback_query(F.data == "admin_qr_delete")
async def admin_qr_delete(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != ADMIN_ID:
        await callback.answer()
        return
    cursor.execute("DELETE FROM bot_settings WHERE key='payment_qr_photo_id'")
    db.commit()
    await state.clear()
    await callback.message.answer(
        "🗑 <b>QR-код удалён.</b>\n\n"
        "При следующей оплате бот временно создаст резервный QR из ссылки оплаты.",
        parse_mode="HTML", reply_markup=admin_menu())
    await callback.answer("QR удалён")
